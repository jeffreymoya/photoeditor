...
import type { IUploadService, PresignUploadResponse, BatchUploadResponse, Job, BatchJob, DeviceTokenResponse } from './port';

/**
 * Upload Service Adapter - concrete implementation of IUploadService
 *
 ...
 */
export class UploadServiceAdapter implements IUploadService {
  ...
  /**
   * Upload image to S3 with resilience policies
   *
   * Wraps S3 upload with retry and circuit breaker for network resilience.
   */
  ...
  async deactivateDeviceToken(deviceId: string): Promise<DeviceTokenResponse> {
...
/**
 * Default instance of upload service
 *
 ...
 */
export const uploadService = new UploadServiceAdapter();

// Initialize with saved URL on app start
uploadService.loadBaseUrl();
