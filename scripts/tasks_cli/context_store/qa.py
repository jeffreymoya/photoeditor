"""
QA baseline management for task context.

Extracted from validation.py for modularization (S3.5).
Handles QA command execution, drift detection, and baseline comparison.
...
class QABaselineManager:
    """
    Manages QA command execution and baseline drift detection.

    Encapsulates all QA-related operations for task context:
    - Command execution with retry and timeout
    - Baseline vs current result comparison
    ...
    def execute_command(
        ...
        if cmd.blocker_id:
            is_blocked, reason = self._check_blocker_status(cmd.blocker_id)
            if is_blocked:
                return {
                    "success": False,
                    "exit_code": None,
        ...
        all_exist, missing = self._verify_expected_paths(cmd.expected_paths)
        if not all_exist:
            return {
                "success": False,
                "exit_code": None,
    ...
    def detect_drift(self, baseline: QAResults, current: QAResults) -> Dict:
        """
        ...
        for cmd_id, current_result in current_results.items():
            ...
            if baseline_result.summary and current_result.summary:
                ...
                if (baseline_summary.lint_errors is not None
                        and current_summary.lint_errors is not None):
                ...
                if (baseline_summary.type_errors is not None
                        and current_summary.type_errors is not None):
                ...
                # Check test failures
                if (baseline_summary.tests_failed is not None
                        and current_summary.tests_failed is not None):
    ...
    def format_drift_report(self, drift: Dict) -> str:
        """
        Format drift detection results as human-readable report.

        Args:
            drift: Output from detect_drift()
        ...
        if drift["improvements"]:
            ...
            for imp in drift["improvements"]:
                ...
                if "delta" in imp:
                    lines.append(
                        f"  ✓ {cmd_id}: {imp_type} "
                        f"(baseline: {imp['baseline']}, current: {imp['current']}, "
                        f"Δ-{imp['delta']})"
                    )

        return "\n".join(lines)

    def _check_blocker_status(self, blocker_id: str) -> tuple[bool, str]:
        """
        ...
        ds = TaskDatastore(self.repo_root)
        tasks = ds.load_tasks()
    ...
    def _verify_expected_paths(self, patterns: list[str]) -> tuple[bool, list[str]]:
        """
...
# These delegate to QABaselineManager but maintain the original API
...
def execute_validation_command(
    ...
    DEPRECATED: Use QABaselineManager.execute_command() instead.
    ...
    """
    manager = QABaselineManager(repo_root)
    return manager.execute_command(cmd, task_id)
...
def detect_qa_drift(baseline: QAResults, current: QAResults) -> Dict:
    ...
    DEPRECATED: Use QABaselineManager.detect_drift() instead.
    Will be removed in Phase 5.
    ...
    """
    # Use a dummy repo_root since drift detection doesn't need it
    manager = QABaselineManager(Path.cwd())
    return manager.detect_drift(baseline, current)
...
def format_drift_report(drift: Dict) -> str:
    ...
    DEPRECATED: Use QABaselineManager.format_drift_report() instead.
    Will be removed in Phase 5.
    ...
    Returns:
        Formatted string report
    """
    # Use a dummy repo_root since formatting doesn't need it
    manager = QABaselineManager(Path.cwd())
    return manager.format_drift_report(drift)
